...
class FreeAnchorRetinaCosineHead(RetinaHead):
    ...
    def __init__(self,
                 ...
                 alpha=0.5,
                 **kwargs):
        super(FreeAnchorRetinaCosineHead,
    ...
    def loss(self,
        ...
        for _, (anchors_, gt_labels_, gt_bboxes_, cls_prob_,
            ...
            with torch.no_grad():
                ...
                #     min=0, max=1)
                # @latstars
                object_box_iou_clamp = ((object_box_iou - (t1+t2)/2.0) / (t2-t1)).clamp(min=-0.5, max=0.5)
                object_box_prob = ( 1 + torch.sin( np.pi * object_box_iou_clamp ) )/2.0
