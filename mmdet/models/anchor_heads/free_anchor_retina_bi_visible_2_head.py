...
class FreeAnchorRetinaBiVisible2Head(RetinaBiVisibleHead):
    ...
    def __init__(self,
                 ...
                 loss_weight=0.4,
                 **kwargs):
        super(FreeAnchorRetinaBiVisible2Head,
    ...
    def loss(self,
        ...
        num_pos = 0
        positive_losses = []
        for _, (anchors_, gt_labels_, gt_bboxes_, gt_bboxes_visible_, cls_prob_,
                bbox_preds_) in enumerate(
                    zip(anchors, gt_labels, gt_bboxes, gt_bboxes_visible, cls_prob, bbox_preds)):
            # gt_labels_ -= 1
            ...
            matched_object_targets = bbox2delta(
                matched_anchors,
                gt_bboxes_visible_.unsqueeze(dim=1).expand_as(matched_anchors),
                self.target_means, self.target_stds)
            ...
            num_pos += len(gt_bboxes_visible_)
        ...
        losses = {
            'positive_bag_visible_loss': positive_loss*self.loss_weight,
            'negative_bag_visible_loss': negative_loss*self.loss_weight
        }
        return losses
